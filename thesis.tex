...
% ------------------------------------------------------------------
\newcommand{\TITLE}{WebSocket:}
%\newcommand{\FTITLE}{Ohjelmistoprosessit mänteille:}
%\newcommand{\STITLE}{Den stora stygga vargen:}
\newcommand{\SUBTITLE}{Development and Deployment of a Fine-Grained Messaging System}
%\FSUBTITLE}{Uusi organisaatio, uudet pyörät}
...
\newcommand{\INSTRUCTOR}{Andrey Ivannikov, Ph.D}
...
\newcommand{\KEYWORDS}{WebSocket, web application development, web performance optimization, containerized virtualization}
...
\newcommand{\AUTHOR}{Ásgeir Bjarnason}
...
% Abstract in English
% ------------------------------------------------------------------
\thesisabstract{english}{WebSocket is a standard that provides full-duplex communication channels over a single TCP connection between clients and servers. It was designed to be a standard for real-time communication on the web and coexist with the current infrastructure of the web. However, network intermediaries, like proxies and load balancers, usually have to be configured specially to allow long-lived connections like the ones WebSocket provides. This can introduce challenges when deploying WebSocket infrastructure for production use.
\\ \\
This thesis presents a messaging system that allows a web chat client to communicate with a back end service using the WebSocket protocol rather than HTTP-based solutions. The system is designed based on the principles of microservices consisting of loosely coupled and fine-grained components. Results from a bench marking test are presented for the identification of bottlenecks in the messaging system and as a basis for capacity planning. Finally, WebSocket deployment considerations are presented through the experimentation with containerized virtualization that provides suitable run-time environments for microservices architecture.
\\ \\
WebSocket substantially simplifies the development of applications with real-time functionality compared to previous approaches. the main obstacle towards greater adoption of WebSocket is the current infrastructure of the Internet that is predominantly configured for HTTP traffic and tends to block any traffic that behaves differently. Increased usage of HTTP/2 will most likely benefit WebSocket as it requires networks intermediaries to be configured for long-lived connections. The main challenge when using containerized virtualization to provide run-time environments for WebSocket applications is that there are usually more intermediaries that require special configuration. The results from this thesis are seen as a starting point for further development of the WebSocket messaging system and an opportunity to get a better understanding of operational challenges related to WebSocket-based infrastructure.}
...
I would like to thank my supervisor, Professor Jukka K. Nurminen, for his valuable guidance throughout the thesis writing process. Andrey Ivannikov, Arturs Polis, Kristoffer Snabb and other employees of IDT Messaging deserve special thanks for their backing during my traineeship. Finally I would like to thank my girlfriend, Ásta Bjarnadóttir, for her support.
\vskip 10mm
...
\input{3microservices}

\input{4websocketMessagingSystem}

\input{5webSocketApplicationDeployment}
