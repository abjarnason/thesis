...
What kind of machines where used for the benchmarking tests. What version of Node was used (why did I pick that version for testing -> stability and the version that is provided in the repostitories of Ubuntu, Debian, CentOS etc.). Where is the bottle neck in this system -> not in the memory usage so it makes more sense to "scale out" than "scale up". How did the benchmarking take place -> new client connection every 2 seconds until clients start to drop connections (why did that happen..). Used iptraf to measure the data sending rate. Why did I choose to measure the things presented here? Do we really benefit from using more cores?! -> does not seem to be so (something about the single threadedness of Node and how that limits something..)
